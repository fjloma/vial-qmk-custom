...
//from keymap.c
extern bool user_keys(uint16_t keycode);
...
void print_chord(chordmod_t* chord) {
	...
	for (int i = 0; i < chord->size; i++) {
		buff[i] = chord->pressed[i]?'X':'_';
	}
	buff[chord->size] = 0;
	uprintf("=CHORD%c timer(%ld) %ld, count %ld %s\n", chord->issued?'*':' ', chord->timer, timer_elapsed32(chord->timer), chord->count, buff);
}
#endif
            ...
            if (check_chord_issued(chord)) {
                ...
                if (chord->count == 1) {
                    for (int i = 0; i < chord->size; i++) {
						...
						#endif
                        if (chord->pressed[i]) {
							if (user_keys(chord->pressed[i]))
								register_code16(chord->pressed[i]);
								#ifdef CONSOLE_ENABLE
